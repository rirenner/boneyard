...
class CreateMo(object):
    ...
    def run_yaml_mode(self):
        ...
        # in yaml mode first check if there is a credentials file.  If there is
        # take the host, username and password from that file.
        if os.path.isfile('credentials'):
            f1 = open('credentials', 'r')
            self.creds = yaml.load(f1)
    ...
    def set_host_user_password(self):
        ...
        # By this time value should be set either in args or from credentials file.
        if self.host == None:
            print "Error: host value is not set"
            exit()
        if self.user == None:
            print "Error: user value is not set"
            exit()
        if self.password == None:
            print "Error: password value is not set"
            exit()
