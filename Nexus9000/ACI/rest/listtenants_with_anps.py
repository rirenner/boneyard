...
formed = json.loads(rsp.content)
#print formed['imdata'][0]['fvTenant']['attributes']['dn']
for i in formed['imdata']:
  tenant = i['fvTenant']['attributes']['dn']
  print tenant

  rsp = sess.get('https://{0}/api/node/mo/{1}.json?query-target=subtree&target-subtree-class=fvAp'.format(apic, tenant), data=json.dumps(userJson, sort_keys=True, indent=4, separators=(',', ':')), verify=False)
  formed = json.loads(rsp.content)
  for j in formed['imdata']:
      anp = j['fvAp']['attributes']['dn'] 
      print "    " + anp
      rsp = sess.get('https://{0}/api/node/mo/{1}.json?query-target=subtree&target-subtree-class=fvAEPg'.format(apic, anp), data=json.dumps(userJson, sort_keys=True, indent=4, separators=(',', ':')), verify=False)
      formed = json.loads(rsp.content)
      for k in formed['imdata']:
         epg = k['fvAEPg']['attributes']['dn'] 
         print "        " + epg 
